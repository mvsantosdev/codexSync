...
There is no linter, formatter, or type-checker configured — CI (`.github/workflows/ci.yml`) only runs `pytest` on `windows-latest` + `macos-latest` × Python 3.11/3.12. Do not add tool config without being asked.
